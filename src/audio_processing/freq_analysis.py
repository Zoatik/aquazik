    ...
    def __get_top_notes(self, fft, mx, prev_notes, top_n=TOP_NOTES) -> list:
        ...
        while( (idx<len(lst_sorted)) and (len(found_notes)<top_n) ):
            try:
                ...
                upper_bound_offset = int((note_above_freq - f - self.freq_dist_per_bin / 2) // self.freq_dist_per_bin)
                lower_bound_offset = int((f - note_below_freq - self.freq_dist_per_bin / 2) // self.freq_dist_per_bin)
                ...
                local_max = max(lst[lower_index:upper_index], key=lambda x: x[1])[1]/mx
                ...
                y_values = [item[1]/mx for item in lst[lower_index:lst_sorted[idx][0]+1]]
                up_slope, intercept = np.polyfit(x_indices,y_values, 1)

                x_indices = np.arange(lst_sorted[idx][0], upper_index)
                y_values = [item[1]/mx for item in lst[lst_sorted[idx][0]:upper_index]]
                ...
                print(f"idx: {idx}, freq: {f}, number: {n}, name: {name}, magnitude: {y}, up_slope: {up_slope}, down_slope: {down_slope}")
                ...
                print(f"area_difference: {area_difference}, square_area: {square_area}, signal_area: {signal_area}")
