...
def rough_start_stop(audio_name="PinkPanther_Trumpet_Only.mp3"):
    ...
    ends_x = []
    ends_y = []
    sound_treshhold = 0.1 * max_energy # originally 0.01
...
def estimate_bpm(x_axis, starts_x, ends_x):
    ...
    distances = [x_axis[starts_x[i+1]] - x_axis[starts_x[i]] for i in range(len(starts_x)-1)]
    ...
    for i in range(len(new_x)):
        if quarter_note == None :
            ...
            loc_max = find_local_maximums(new_x[lower_bound:upper_bound], new_y[lower_bound:upper_bound])
    ...
    print("coeffs: ", coeffs)
    m = np.mean([quarter_note] + [mult_notes[i]/coeffs[i] for i in range(len(mult_notes))])
    print("mean: ", m)
    est_bpm = 1 / m * 60.0
...
norm_signal, smoothed_data, starts_x, ends_x = rough_start_stop("retro_lounge.mp3")
...
quarter_note, bpm = estimate_bpm(smoothed_data[0], starts_x, ends_x)
